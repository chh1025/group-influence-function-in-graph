...
def DataLoader(name, root):
    
    name = name.lower()
    if name in ['cora', 'citeseer', 'pubmed']:
        root_path = './'
        path = osp.join(root_path, root, name)
        dataset = Planetoid(path, name)
    elif name in ['cora_public', 'citeseer_public', 'pubmed_public']:
        root_path = './'
        name = name.split('_')[0]
        path = osp.join(root_path, root, name)
        dataset = Planetoid(path, name)
        ...
        dataset = dataset_heterophily(root=root, name=name)
    elif name in ['actor']:
