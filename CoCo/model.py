...
class Compact_learning(object):
    ...
    def __call__(self, embds):
        b, n = embds.size()
        mu = self.mu.repeat(b, 1).to(device=embds.device, dtype=embds.dtype) # mu=lambda z=y
        # mu = mu / (mu.norm(dim=0, keepdim=True))
        # _embds = embds
        embd_nor = embds / (1e-6 + embds.norm(dim=0, keepdim=True))
        _embds = embds / (1e-6 + embds.norm(dim=0, keepdim=True))
