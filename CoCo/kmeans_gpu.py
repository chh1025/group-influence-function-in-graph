...
def initialize(X, num_clusters):
    ...
    """
    num_samples = len(X)
    indices = torch.as_tensor(np.random.choice(num_samples, num_clusters, replace=False), device=X.device)
...
def kmeans(
    ...
    for i in range(20):
        ...
        if torch.isfinite(dis) and float(dis.detach().cpu()) < dis_min:
            dis_min = float(dis.detach().cpu())
            initial_state_best = initial_state

    if initial_state_best is None:
        initial_state_best = initialize(X, num_clusters)
    ...
    while True:
        ...
        for index in range(num_clusters):
            selected = torch.nonzero(choice_cluster == index, as_tuple=False).view(-1).to(device)
            if selected.numel() == 0:
                continue
